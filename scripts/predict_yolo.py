...
def run_benchmark(predictor: YOLOPredictor, test_images: List[str], args):
    ...
    logger.info(f"  • Detecções/imagem: {metrics['avg_detections_per_image']:.1f}")
...
def visualize_results(
    ...
    for result in results:
        ...
        try:
            ...
            logger.warning(f"⚠️ Erro criando visualização para {result.image_path}: {str(e)}")
...
def main():
    ...
    try:
        ...
        if args.benchmark and results:
            test_images = [r.image_path for r in results if r.image_path][:10]  # Até 10 imagens
            if test_images:
        ...
        if results:
            ...
            total_detections = sum(r.num_detections for r in results)
            images_with_detections = sum(1 for r in results if r.has_detections)
            ...
            logger.info(f"  • Imagens com detecções: {images_with_detections} ({images_with_detections/total_images*100:.1f}%)")
            logger.info(f"  • Detecções/imagem: {total_detections/total_images:.1f}")
            ...
            logger.success(f"✅ Predições concluídas! Resultados salvos em: {output_dir}")
        else:
